...
*
*xco_cp_abap_sql=>constraint->contains_pattern(
*to_upper( sap_table_request ) && '%' ) ).
* DATA(objects) =
*xco_cp_abap_repository=>objects->tabl->where( VALUE #(
* ( name_filter ) ) )->in(
...
* the request comes in with metadata around the actual
*file data,
* extract the filename and fileext from this metadata
*as well as the raw file data.
       ...
       DATA(content_item) INDEX 2.
        IF sy-subrc = 0.
         ...
         DATA(content_dis_item) INDEX 3.
          ...
          ENDIF.
        ENDIF.
        DELETE content FROM 1 TO 4. " Get rid of thefirst 4 lines
...
* Load the data to the table via dynamic internal table
           ...
           @<table_structure>.
            IF sy-subrc = 0.
             ...
             if_web_http_status=>ok
              i_reason = 'Tablenupdated successfully' ).
  ...
  METHOD get_html.
    ...
    | <script id="sap-ui-bootstrap" src="https://sapui5.hana.ondemand.com/resources/sap-uicore.js" \n| &&
    | data-sap-ui-theme="sap_fiori_3_dark" data-sap-ui-xx-bindingSyntax="complex" data-sap-uicompatVersion="edge" \n| &&
